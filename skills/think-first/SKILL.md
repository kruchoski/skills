---
name: think-first
description: Apply mental models before major decisions. Enforces structured thinking before implementation or recommendations.
use_when: User faces a significant decision, architectural choice, prioritization question, strategic trade-off, or when Claude should challenge thinking more rigorously.
effort: normal
---
...
## Integration with Structured Thinking

This skill operationalizes structured thinking principles:
- "Challenge first - Test assumptions and evidence"
- "Multiple perspectives - Present alternatives with trade-offs"
- "Red team - Include skeptical viewpoint"

When these rules apply, use these frameworks.
...
- Strategic choices ("should we pursue X or Y?")
- Prioritization ("what to focus on first?")
- Trade-offs ("speed vs quality", "scope vs timeline")
- Direction changes ("should we pivot?")
- Resource allocation ("where to invest time?")
- Stakeholder positioning ("how to frame this?")
...
**Use when:** Choosing between options, allocating time, or deciding what to build/pursue.

**Process:**
1. List the options being considered
2. For each option, identify what you're giving up by choosing it
3. Quantify where possible (time, money, opportunities, relationships)
...
|---------------|-------------|
| "What should we build/pursue?" | First Principles, Inversion |
...
| "Are we making assumptions?" | First Principles |
| "What could go wrong?" | Inversion |

## Combining with Epistemic Analysis

For high-stakes decisions, use think-first models THEN run epistemic-analysis agents:
1. **Think-first** structures the decision and surfaces trade-offs
2. **Epistemic agents** stress-test from five temperament perspectives

This provides both analytical rigor and cognitive diversity.

## Invocation

Automatic triggers:
- Significant decision detected in conversation
- User asks "what do you think about X" where X has trade-offs
- User says "help me decide", "should I", "which option"

Manual: `/think-first` or "apply a mental model to this"

## Success Criteria

- [ ] Decision moment detected before implementation
- [ ] Appropriate model selected for decision type
- [ ] Model output is complete (not abbreviated)
- [ ] Analysis informs the recommendation
